...
const Contact: React.FC = () => {
    ...
    const InputField: React.FC<{ label: string, name: string, type: string, value: string, onChange: (event: React.ChangeEvent<HTMLInputElement>) => void, min?: string }> = ({ label, name, type, value, onChange, min}) => {
        return (
            <div className="text-start w-full pr-5">
                <label htmlFor={name} className="text-sm text-[#2C2F24] font-semibold">{label}</label>
                <input
                    type={type}
                    id={name}
                    name={name}
        ...
        <div className="body flex flex-col">
            <div className="section flex-col relative mb-48">
               <div className="text w-2/5 text-center">
                    ...
                    <p className="description text-normal">We consider all the drivers of change gives you the components you need to change to create a truly happens.</p>
                </div>
            </div>
            <div className="form absolute inset-x-1/2 -translate-x-1/2 -translate-y-2/5 shadow-xl w-full md:w-1/2">
                <form onSubmit={handleSubmit} className="w-full pl-5 py-5 bg-white rounded-md shadow-md space-y-4">
                    ...
                    <InputField label="Subject" name="subject" type="text" value={formData.subject} onChange={handleInputChange}/>
                    <InputField label="Message" name="message" type="text" value={formData.message} onChange={handleInputChange}/>
                    <div className="button w-full flex justify-center pr-5">
                        <button type="submit" className="w-full text-lg font-semibold red_btn">Send</button>
                    </div>
                </form>
            </div>
